...
func GetWebRtcAccess(c *fiber.Ctx) error {
	isSender, _ := strconv.ParseBool(c.Params("isSender"))

	userID := c.Params("userID")
	peerID := c.Params("peerId")

	if !isSender {
		userID = fmt.Sprintf("%s.false", userID)
	} else {
		userID = fmt.Sprintf("%s.true", userID)
	}
	...
	// return c.JSON(fiber.Map{"status": "success", "message": "Created user", "data": Sdp{Sdp: Encode(answer)}})

	return c.JSON(fiber.Map{"status": "success", "message": "Created WebRtc Access", "data": user})
}
